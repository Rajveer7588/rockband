<?php include_once('session_destroy.php') ;
    ...
    $fetch_cat = mysqli_query($conn, "SELECT * FROM `fleet_category` WHERE admin_id = 1");



    $admin_id = $_SESSION['admin_id'];

    $fetch = mysqli_query($conn,"SELECT * FROM `fleet` WHERE id = '".$product_id."'");
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="down_header">
        <h1>Fleet <small>Update your Content Easily</small></h1>
        <a href="add-fleet-category.php" class="cat_button">Add New Category</a>
      </div>
      ...
      <div class="row">
          ...
          <div class="image_holder">
            ...
            <?php if(@$image==""){ ?>
                ...
                <?php }else{ ?>
                  ...
                  <img src="../uploads/fleet/<?= $category_id ?> /<?php echo @$image ?>" width="200" height="200">
        ...
        // Look for a GET variable page if not found default is 1.     
            ...
            $fetch_pro = mysqli_query($conn,"SELECT * FROM `fleet` LIMIT $start_from, $limit");
				...
				<table class="responsive-table">
					...
					<tbody>
						...
						<tr>
							...
							<form method="post" action="add-fleet-details.php?id=<?php echo $id; ?>">
				    ...
				    <ul class="pagination">
                      <?php  
                      	if($nums>0)
            			{
                            ...
                            if($pn >=2){
                            ?>
                                 <li class='active'><a href="add-fleet-details.php?page=<?= $pn-1; ?>" id="pagination_txt">Prev</a></li>
                            <?php
                            }
                            for ($i=1; $i<=$total_pages; $i++) {
                              if ($i==$pn) {
                                ?>
                                  <li class='active'><a href='add-fleet-details.php?page=<?= $i ?>' id="pagination_txt"><?php echo $i; ?></a></li>
                                <?php
                              }            
                              else{
                                ?>
                                    <li><a href='add-fleet-details.php?page=<?= $i ?>' id="pagination_txt">
                                                                    <?php echo $i; ?></a></li> 
                                <?php
                                } 
                            }
                            if($pn<=$total_pages){
                            ?>
                                 <li class='active'><a href="add-fleet-details.php?page=<?= $pn+1; ?>" id="pagination_txt">Next</a></li>
                            <?php
                            }
            			}
                        ?>
...
if(isset($_POST['about_submit']))
{
    ...
    $ip = getClientIP();
    $target_dir = '../uploads/fleet/'.$category_id.'/';
    ...
    $file_array = explode(".",$image);
      ...
      if( in_array($imageFileType,$extensions_arr) ){
        ...
        $insert = mysqli_query($conn,"INSERT into fleet(admin_id,category_id,image,heading,description,short_content,product_size,inserted_date,service_ip)
        ...
        // die;
          $update = mysqli_query($conn, "UPDATE fleet SET category_id= '".$category_id."', image='".$image."' , heading='".$heading."', description='".$description."', short_content='".$short_content."', product_size= '".$product_size."', updated_date=now(), service_ip='".$ip."' WHERE id='1' AND admin_id='".$admin_id."'") or (mysqli_error($conn));
          move_uploaded_file($_FILES['image']['tmp_name'],$target_dir.$image);
          if($update==true)
          {
            ...
            ?>
              <script>
              window.location = '<?php echo $hostname."wocms/add-fleet-details.php" ?>';
              </script>
            <?php
          }
          else
