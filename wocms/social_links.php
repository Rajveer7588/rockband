  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="instragram" id="instragram" placeholder="Instagram" value="<?php echo @$instragram ?>">
        </div>
        
        <div class="form-group">
        <label for="exampleInputEmail1"><i class="fa fa-google-plus-square" aria-hidden="true"></i> Google Plus</label>
        <input type="text" class="form-control" name="google_plus" id="google_plus" placeholder="Google Plus" value="<?php echo @$google_plus ?>">
        </div>

        <div class="form-group">
        <label for="exampleInputEmail1"><i class="fa fa-pinterest" aria-hidden="true"></i>Pinterest</label>
...
if(isset($_POST['social_submit']))
{
	...
	if($row==0)
	{
		...
		$insert1 = mysqli_query($conn,"INSERT into social_links(admin_id,facebook,twitter,linkdin,instragram,pinterest,google_plus,social_status,social_insert_date,social_ip) VALUES('".$admin_id."','".$facebook1."','".$twitter1."','".$linkdin1."', '".$instragram1."', '".$pinterest."', '".$google_plus1."', 1 , now() ,'".$ip."')") or (die(mysqli_error($conn)));
	...
	if($row>0)
	{
		...
		{
			$update = mysqli_query($conn,"UPDATE social_links SET facebook='".$facebook1."', twitter='".$twitter1."', linkdin='".$linkdin1."', instragram='".$instragram1."', pinterest='".$pinterest."', google_plus='".$google_plus1."', social_update_date=now() , social_ip='".$ip."' WHERE social_id='".$loop['social_id']."' AND admin_id='".$admin_id."'") or (die(mysqli_error($conn))) ;
		}
		if($update)
		{
