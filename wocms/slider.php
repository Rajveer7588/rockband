...
<?php
$file_query = mysqli_query($conn,"SELECT count(image_id) as c_no,sum(file_size) as file FROM slider WHERE admin_id='".$admin_id."'");
...
<div class="image_upload_div">
	...
	<form action="slider_upload.php" method="post" class="dropzone" id="HomeSlideGallery">
...
{
	echo "<p class='faa-flash animated rishi'>Maximum number of images for Slider is 6.For uploading more images,please delete some images first.</p>";
}
 ?>
...
<a href="slider.php" class="btn btn-success btn-outline-rounded green" onClick="return gallery()">Refresh Gallery <i class="fa fa-sliders" aria-hidden="true"></i></a></p>
...
$fetch = mysqli_query($conn,"SELECT * FROM slider WHERE admin_id='".$admin_id."' ORDER BY menu_order ASC");
...

{
    ...
    <li class="img_li" id="menu-order-<?php echo $row['image_id'] ?>"><img src="../uploads/slider/<?php echo $row['file_name']; ?>" />
	<div class="img_dt">
	
	<input type="text" name="img_<?php echo $row['image_id'] ?>" placeholder="" value="<?php echo @$row['caption'] ;?>" /><div class="deleteCheck_box">
	<input type="checkbox" name="del_immg[]" class="del_imgg" id="del_imgg" value="<?php echo $row['image_id'];?>" />
	</div><button class="del" type="submit" value="<?php echo $row['image_id'] ?>">Delete</button></div>
...

{
...

	{
		...
		$update = mysqli_query($conn,"UPDATE slider SET caption='".$cap."', updated_image_date=now() WHERE image_id='".$cap_id."' AND admin_id='".$admin_id."'") or (mysqli_error($conn));
