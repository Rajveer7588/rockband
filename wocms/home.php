...
$admin_id = $_SESSION['admin_id'];
  $fetch = mysqli_query($conn,"SELECT * FROM home_details WHERE admin_id='1'");
...
if(isset($_POST['home_details_submit']))
{
	...
	$insert = mysqli_query($conn,"SELECT * FROM home_details WHERE admin_id=1");
	$row = mysqli_num_rows($insert);
	
	if($row==0)
	{
		
		$insert = mysqli_query($conn,"INSERT into home_details(admin_id,home_heading,home_content,	home_video_url,home_type,home_no_of_bedrooms,home_no_of_sleeps,home_no_of_baths,home_det_insert_date,home_det_ip) VALUES('".$admin_id."','".$pro_heading."','".$pro_content."','".$pro_video_url."','".$pro_type."','".$pro_no_of_bedrooms."','".$pro_no_of_sleeps."','".$pro_no_of_baths."', now() ,'".$ip."')");
	...
	if($row>0)
	{
		...
		{
			$update = mysqli_query($conn,"UPDATE home_details SET home_heading='".$pro_heading."', home_content='".$pro_content."', home_video_url='".$pro_video_url."', home_type='".$pro_type."', home_no_of_bedrooms='".$pro_no_of_bedrooms."', home_no_of_sleeps='".$pro_no_of_sleeps."', home_no_of_baths='".$pro_no_of_baths."', home_det_update_date=now() , home_det_ip='".$ip."' WHERE home_id='".$loop['home_id']."' AND admin_id='1'") or (mysqli_error($conn));
		}
		if($update)
		{
