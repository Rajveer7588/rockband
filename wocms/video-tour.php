...
$insert = mysqli_query($conn,"INSERT into video_detail(admin_id,img,cart_type,c_name,heading,c_review,c_place,added_date) VALUES('".$admin_id."','".$img_name."','".$c_type."','".$c_name."','".$heading."','".$c_review."', '".$c_place."', now())");
        ...
        else
  {
      echo "<script>alert('Size of image is greater than 4MB, Please insert image than 4 Mb.');</script>";
            ?>
            <script>
            window.location = 'video-tour.php';
            </script>
            <?php
  }
    if($insert)
...
$(document).ready(function(e) {
    ...
    $('#add_areview').click(function(){
        var but = $('#add_areview').val();
            ...
            if(but== 'Add Video Link' )
                {
                    $('#add_areview').val('Close X');
                }
            else
                {
                    $('#add_areview').val('Add Video Link');
                    $('#add_rev').hide();
                }
        });
        return false;
...
?>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>Video Links<small>Add Video Links From Here</small> </h1>
      <hr>
      <input type="button" class="btn btn-success green" id="add_areview" value="Add Video Links" style="margin:10px;"/>
      <div id="add_rev">
 ...
 -->
   <div class="form-group">
          ...
          <input type="text" name="heading" class="form-control" id="home_heading" placeholder="Video Link"/>
        </div>
        ...
        <!-- <div class="editor">
         ...
         <!--<div class="form-group">
          ...
          <input type="text" name="c_place" class="form-control" id="home_heading" placeholder="Customer Place"/>
        </div>-->
        ...
        <!-- <div class="col-md-4">
            <div class="input_rates">
              <div class="form-group">
                ...
                <input type="text" name="c_type" class="form-control" id="pro_new_rate_desc" placeholder="Car type" />
              </div>
            </div>
          </div> -->
        ...
        <!-- <div class="col-md-4">
            <div class="input_rates">
              <div class="form-group">
                ...
                <input type="text" name="c_place" class="form-control" id="pro_new_rate_desc" placeholder="Price /day" />
              </div>
            </div>
          </div>
           ...
           <div class="input_rates">
              <div class="form-group">
                <label for="exampleInputEmail1">CDW Rate</label>
                <input type="text" name="c_review" class="form-control" id="pro_new_rate_week_nt" placeholder="CDW Rate" />
              </div>
            </div>
           ...
           <div class="input_rates">
              <div class="form-group">
                <label for="exampleInputEmail1">Child Seat Rate</label>
                <input type="text" name="c_name" class="form-control" id="pro_new_rate_weekly_nt" placeholder="Child Seat rate" />
              </div>
            </div>
...
?>
            <div class="prop_type">
              <section>
                ...
                <?php while($fetc = @mysqli_fetch_assoc($pro_fin)){
                    $pro_head = $fetc['property_heading'];
                    $pro_id = $fetc['property_id'];
                    ?>
                 <option value="<?php echo $pro_id; ?>"><?php echo $pro_head ;?></option>
                <?php } ?>
                </select>
              </section>
...
$page_query=mysqli_query($conn,"SELECT id FROM video_detail WHERE admin_id='".$admin_id."'");
...
$start= ($page - 1) * $per_page; //start list of user from zero becoz php starts from zero
  ...
  $fetch11 = mysqli_query($conn,"SELECT * FROM video_detail WHERE admin_id='".$admin_id."'  ORDER BY id DESC LIMIT $start, $per_page");
       $num11 = mysqli_num_rows($fetch11);
       if($num11>0)
       {
    ?>
        <h3 class="head text-center">Video Links Details</h3>
        <table class="responsive-table">
          <thead>
            <tr>
              ...
              <th scope="col">CDW Rate </th>
              <th scope="col">Child Seat Rate </th>
    ...
    ?>
        ...
        <!-- <td data-title="Start Date"><?php echo $c_type ; ?></td> -->
        <!-- <td data-title="Start Date">$<?php echo $c_place ; ?></td>
        <td data-title="Start Date">$<?php echo $c_review ; ?></td>
        <td data-title="Start Date">$<?php echo $c_name ; ?></td> -->
        <!-- <td scope="row"><img src="<?php echo $cust_image ;?>" width="50px" height="50px"></td> -->
        <!-- <form method="post" action="edit_tour.php">
        <td data-title="Edit" >
   ...
   if(isset($_POST['delete_rate_button']))
{
    ...
    $dt = mysqli_query($conn, "SELECT property_id,img FROM video_detail WHERE id='".$delete_id."'");
    $dell = mysqli_fetch_assoc($dt);
    $del_file = $dell['img'];
    //$pro_id = $dell['property_id'];
    $delete = mysqli_query($conn, "DELETE FROM video_detail WHERE id=$delete_id");
    if($delete)
        {
            ...
            echo "<script>window.location = 'video-tour.php'</script>";
        }
}
