<?php include_once('session_destroy.php') ;
	...
	if(!empty($updated_id)){
		$fetch12 = mysqli_query($conn,"SELECT * FROM `city_category` WHERE id ='".$updated_id."' AND admin_id='".$admin_id."'");
...
<div class="wrapper">
	<div class="content-wrapper">
		<section class="content-header">
			<h1>City<small>Add Category Easily</small> </h1>
			<hr>
			...
			<div class="form-group">
				...
				<input type="text" name="heading" class="form-control" id="property_heading" placeholder=" Enter new City" value="<?= $heading; ?>" required/>
				</div>
				
				<p class="text-center">
				<button type="submit" name="submit" class="btn btn-success btn-outline-rounded green"> Add City<span style="margin-left:10px;" class="glyphicon glyphicon-send"></span></button>
				</p>
			</form>
		<?php
			//  echo "SELECT * FROM aid_listing WHERE admin_id='".$admin_id."' and property_id not in (7,15,8)";

			$fetch11 = mysqli_query($conn,"SELECT * FROM `city_category` WHERE admin_id='".$admin_id."'");
					...
					<tbody>
						<?php
							...
							<tr>
								...
								<form method="post" action="add-city-category.php?id=<?php echo $id; ?>">
...
<!-- 
	...
	// }
		...
		// if(empty($updated_id) && $upload == 1){
            ...
            // 
                // $insert = mysqli_query($conn,"INSERT into `category`(admin_id,heading, category_image,status,inserted_date,ip) VALUES('".$admin_id."','".$heading."', '".$image_name."', '".$pro_status."', now() ,'".$ip."')");

                $insert = mysqli_query($conn,"INSERT into `city_category`(admin_id,heading, status,inserted_date,ip) VALUES('".$admin_id."','".$heading."', '".$pro_status."', now() ,'".$ip."')");
                ...
                if($insert)
                {
                    ...
                    ?>
                    <script>
                    window.location = 'add-city-category.php';
                    </script>
                    <?php
                }
                else
        ...
        //========
			...
			$insert = mysqli_query($conn, "UPDATE `city_category` SET `admin_id`='$admin_id',`heading`='".$heading."', `inserted_date`= now(),`ip`='".$ip."' WHERE `id`='$updated_id' AND `admin_id`='$admin_id' ");
            if($insert)
			{
...
if(isset($_POST['delete_pro_button']))
{
	...
	$delete = mysqli_query($conn, "DELETE FROM `city_category` WHERE id = '".$delete_id."' AND admin_id=$admin_id");
	if($delete)
		{
