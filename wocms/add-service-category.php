<?php include_once('session_destroy.php') ;
	...
	if(!empty($updated_id)){
		$fetch12 = mysqli_query($conn,"SELECT * FROM `category` WHERE id ='".$updated_id."' AND admin_id='".$admin_id."'");
...
<div class="wrapper">
	<div class="content-wrapper">
		...
		<?php
			...
			$fetch11 = mysqli_query($conn,"SELECT * FROM `category` WHERE admin_id='".$admin_id."'");
					...
					<tbody>
						<?php
							...
							<tr>
								...
								<form method="post" action="add-service-category.php?id=<?php echo $id; ?>">
	...
	if($imageFileType != "jpg" && $imageFileType != "jpeg" && $imageFileType != "gif" && $imageFileType != "png" && $imageFileType != "webp"){
		...
		if(empty($updated_id) && $upload == 1){
            ...
            // 
                ...
                // $insert = mysqli_query($conn,"INSERT into `category`(admin_id,heading, status,inserted_date,ip) VALUES('".$admin_id."','".$heading."', '".$pro_status."', now() ,'".$ip."')");
                ...
                if($insert)
                {
                    ...
                    ?>
                    <script>
                    window.location = 'add-service-category.php';
                    </script>
                    <?php
                }
                else
        ...
        //========
			$insert = mysqli_query($conn, "UPDATE `category` SET `admin_id`='$admin_id',`heading`='".$heading."', `category_image`='".$image_name."', `inserted_date`= now(),`ip`='".$ip."' WHERE `id`='$updated_id' AND `admin_id`='$admin_id' ");
			// $insert = mysqli_query($conn, "UPDATE `category` SET `admin_id`='$admin_id',`heading`='".$heading."', `inserted_date`= now(),`ip`='".$ip."' WHERE `id`='$updated_id' AND `admin_id`='$admin_id' ");
            if($insert)
			{
...
if(isset($_POST['delete_pro_button']))
{
	...
	$delete = mysqli_query($conn, "DELETE FROM `category` WHERE id = '".$delete_id."' AND admin_id=$admin_id");
	if($delete)
		{
