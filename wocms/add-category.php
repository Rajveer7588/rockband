					...
					<tbody>
						<?php
							...
							<tr>
								...
								<form method="post" action="add-category.php?id=<?php echo $id; ?>">
...
if(isset($_POST['submit']))
{
	...
	if($imageFileType != "jpg" && $imageFileType != "jpeg" && $imageFileType != "gif" && $imageFileType != "png" && $imageFileType != "webp"){
		...
		if(empty($updated_id) && $upload == 1){
			...
			echo "UPDATE `category` SET `admin_id`='$admin_id',`heading`='".$heading."', `category_image`='".$image_name."', `inserted_date`= now(),`ip`='".$ip."' WHERE `id`='$updated_id' AND `admin_id`='$admin_id' ";
			die;
